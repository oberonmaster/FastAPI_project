...
from sqlalchemy import Table, Column, Integer, String, ForeignKey, Boolean, DateTime, TIMESTAMP, func, CheckConstraint
...
# association table для many-to-many: Meeting <-> User
...
class User(Base):
    ...
    admin_of = relationship(
        ...
        uselist=False,
        foreign_keys="Team.team_admin",
        lazy="selectin",
    )

    # обратные связи для задач / оценок / встреч
...
class Task(Base):
    ...
    # оценки: одну задачу могут оценивать (например) несколько человек — связь one-to-many
...
class Team(Base):
    ...
    admin = relationship(
        "User",
        back_populates="admin_of",
        foreign_keys=[team_admin],
        uselist=False,
        lazy="joined",
    )
    ...
    members = relationship(
        "User",
        back_populates="team",
        foreign_keys="User.member_of_team",
        lazy="selectin",
...
class Evaluation(Base):
    ...
    __table_args__ = (
        CheckConstraint("evaluation_value >= 1 AND evaluation_value <= 5",
                        name="ck_evaluation_value_range"),
    )
